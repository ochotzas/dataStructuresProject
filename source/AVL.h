...
public:
    ...
    int height(node* r)
    {
        ...
        int left1, right1;
        left1 = 1 + height(r->left);
        right1 = 1 + height(r->right);
...
node* AVL::rebalance(node*& root,string word)
{
    ...
    if(root == nullptr)  // Αν η ρίζα που ελέγχουμε είναι nullptr (πχ αν είναι φύλλο σε κάποιο υποδέντρο)
    ...
    {
        // Δημιουργούμε νέο (προσωρινό) κόμβο ο οποίος θα πάρει τιμή από την perform_rotations
        // (αφου γίνουν δηλαδή οι κατάλληλες περιστροφές και η ρίζα του υποδέντρου αλλάξει)
